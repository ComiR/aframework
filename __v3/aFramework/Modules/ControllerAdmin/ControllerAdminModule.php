				...
				if (is_dir($modulesDir)) {
					...
					while ($f = readdir($dh)) {
						if (!in_array($f, self::$notModules) and is_dir($modulesDir . $f)) {
							$newModule = array(
								'name'		=> $f, 
								'html_id'	=> strtolower(ccFix($f)), 
								'thumb'		=> file_exists($modulesDir . $f . '/thumb.png') ? WEBROOT . $site . '/Modules/' . $f . '/thumb.png' : WEBROOT . 'aFramework/Files/icons/80x80-round-gradient.png'
							);

							if (array_key_exists($f, $modulesInController)) {
								$availableModules[$f]			= $newModule;
								$availableModules[$f]['in_use']	= true;
							}
							else {
								$usedModules[$f]			= $newModule;
