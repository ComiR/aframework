...
jQuery.fn.pixasticEditor = function (conf, cb) {
	...
	var config = jQuery.extend({
		...
		noOptionsText:		'No options are required for this effect but please confirm it.', 
		previewWidth:		160, 
		previewHeight:		120, 
		fullPreview:		false, 
		ignoreEffects:		{}
						...
						if (options[i].min && jQuery.fn.slider) {
							jQuery('<div class="' + config.className + '-slider"><div class="ui-slider"><div class="ui-slider-handle"></div></div></div>')
								...
								.slider({
									...
									startValue:	parseInt(newFormItem.find(':input').val(), 10), 
									slide:		function (e, ui) {
										ui.handle
											.parents('p')
											.eq(0)
