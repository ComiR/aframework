	...
	<?php foreach ($item as $field) { ?>
		<p>
			<label>
				...
				<?php } else { ?>
					<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo escHTML($field['value']); ?>"<?php echo ($field['name'] == Router::$params['table_name'] . '_id' and empty($field['value'])) ? ' readonly="readonly"' : ''; ?>/>
				<?php } ?>
			</label>
		</p>
	<?php } ?>
